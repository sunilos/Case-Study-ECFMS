...
public class SendboxListCtl extends BaseCtl {

	private static Logger log = Logger.getLogger(SendboxListCtl.class);
	...
	protected BaseModel populateModel(HttpServletRequest request) {
		...
		UserBean userBean = (UserBean) request.getSession()
				.getAttribute("user");
		model.setReceiver(DataUtility.getString(request
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op)
				...
				if (OP_SEARCH.equalsIgnoreCase(op)) {
					...
					ServletUtility.redirect(ORSView.SENDBOX_LIST_CTL, request,
							response);
					return;
				}
	...
	protected String getView() {
		return ORSView.SENDBOX_LIST_VIEW;
	}
